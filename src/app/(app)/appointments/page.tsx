...
export default function AppointmentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const snap = await getDocs(query(
      ...
      where('start_time', '<=', wEnd.toISOString()),
      orderBy('start_time')
    ))
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto w-full">
      ...
      {loading ? <LoadingState /> : dayAppointments.length === 0 ? (
        <EmptyState icon={CalendarDays} title="Nessun appuntamento" description="Nessun appuntamento per questo giorno." />
      ) : (
